...
### Model Selection
Random Forests: Chosen because it adds more randomness to our sample data the way we would expect to see in the real world. We consider a total of 83 features in the final model design so using random forests helps subset the number of features. Additionally, despite logit models being more approporiate for a binary outcome variable, the majority of our features are factors with a large number of categories, such as DIAG1 which includes 808 possible diagnoses. This combined with the number of potentially relevant variables makes random forest modelling the most reasonable choice.

**Model Training:** The data show that opioid prescriptions are relatively stable in 2018-2019 relative to the previous years. There might be a lag in broader adoption of the 2016 CDC guidelines and the prescriptions drop from 2016-2018. Therefore, 2018-2019 data were taken as “ideal” prescription rates and the model was trained on this data using an 80/20 train test split. 
We use the F1 score metric to identify the strength of the model at different thresholds, finding that 0.17 yielded the maximum, where $$F1 =\frac{2*(precision*Recall)}{(Precision + Recall)}$$

The model was then run on data prior to the CDC guidelines (Jan 2013 - Mar 2016) to identify the false negatives, which are comprised of the individuals who would not have been prescribed had the CDC guidelines existed at the time. We will study the summary statistics of these overprescribed individuals to determine if there are any systemic differences between them and the accurately non-prescribed group.
