...
## Post Regulation modelling for 2018-2019
...
## F1 test to choose threshold
x <- factor(testdata$opioid)
y <- factor(testdata$predict_opioid)

confusionMatrix(y, x, mode = "everything", positive="1") # Best cutoff = 0.17

table(real_opioid=testdata$opioid, predict_opioid=testdata$predict_opioid)

# Null model is 95% accurate

## ROC Curve (Now shown in MD file but used for picking threshold)
# w <- (testdata$opioid)
...
```{r, echo=FALSE}
testdata = testdata %>%
  mutate(predict_opioid = ifelse(predict_opioid >= 0.4, 1, 0), 
