...
Among these three different methods, the tree regression model presents the best out-of-sample performance. The root mean squared error for this tree model equals 60,000.
...
```{r , include=FALSE}
################################### Read dataset
CAhousing = read.csv("C:/Users/ACER/Desktop/Stats Learning/CAhousing.csv")
...
ggplot(CA_tracts) +
  ...
  labs(y = "", x = "", title = "Original values")+
...
ggplot(CA_tracts) +
  ...
  theme_bw() +
  theme(legend.key.size = unit(0.5, 'cm'),legend.position = c(0.8, 0.8))+
  labs(y = "", x = "", title = "Predicted Values")+
  theme(legend.title = element_blank(),legend.text = element_text(size = 8, colour = "black",face = "bold"))+
  theme(axis.text.y = element_blank(),axis.text.x = element_blank())+
  theme(plot.title = element_text(hjust = 0.5))
```
